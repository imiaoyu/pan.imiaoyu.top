...
if (sessionStorage.getItem('theme_link')=="./style/eui_dark/index.css") {
...
export default {
  ...
  data() {
    return {
      myValue: this.value,
      file_base_url: "http://localhost/static/uploads/image/", //网站链接，与返回文件地址拼接形成完整地址
      init: {
        selector: "Editor", //选择HTML元素
        language: "zh_CN", //语言设置
        //disabled:true, //禁用
        // skin_url: "/tinymce/skins/ui/oxide-dark", //主题样式
        height: 300, //高度
        ...
        images_upload_handler: (blobInfo, success, failure) => {
          Faxios.upload('image',blobInfo.blob()).then((res)=>{
            success(this.file_base_url+res.data)
          }).catch(()=>{
            failure("上传失败");
          })
        },
